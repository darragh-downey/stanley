...
func TestLinearParser(t *testing.T) {
	tt := []struct {
		name     string
		stream   []byte
	...
	for _, testCase := range tt {
		response, err := app.LinearParser(testCase.stream)
		// we received an error for a test case that should pass
		if err != nil && testCase.expected == true {
			t.Error(err)
		}

		// expect responses to match for an expected passing case
		if !compare(response.Responses, testCase.response) {
			t.Errorf("Response does not match expected for test case '%s': %v %v\n", testCase.name, response, testCase.response)
		}
	}
}
